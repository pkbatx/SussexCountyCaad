...
Shift the operator workflow to incident-first views with append-only rollup
history, conservative and explainable grouping, grounded extraction backed by
local reference data when available, incident-aware notification deduping, and
bounded feedback signals. Preserve the existing ingestion, staged pipeline,
strict JSON validation, and UI while improving stability and reducing noise.
...
**Performance Goals**: Incident feed reflects new calls within seconds under local
load  
**Constraints**: Local-first, read-only calls input, strict JSON validation with
repair retries, conservative grouping, append-only rollups, incident-level
notification dedupe, provider abstraction maintained  
