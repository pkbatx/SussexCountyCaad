...
- `hooks/` — `useSseStatus` (auto-reconnecting SSE), `useDetailCache` (per-id TTL cache), `useHashRoute`, `useMapViewState`, `useSignals(callId, refreshToken)` (per-call agentic-loop signals from `/api/signals?call_id=…`), `useMapbox({ containerRef, viewState, onViewChange, fitBoundsOnInit })` (shared Mapbox init/cleanup; returns the adapter once `load` fires).
...
`legacy-styles.css` has been pruned to only the rules backing surviving components that haven't been redesigned (`TopFilterBar`, `FilterPanel`, `DigestColumn`, and the `map-*` wrapper classes the global `MapView` still uses). The legacy `IncidentsBoard`, `NotificationsView`, `CallsList`, `AudioPlayer`, `Timeline*`, `MetricsPanel`, `SummaryPanel`, and `AppLayout` components were removed entirely along with their CSS.
