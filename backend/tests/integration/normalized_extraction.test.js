...
function setupDb() {
  ...
  db.prepare(
    "INSERT INTO calls (call_id, source_path, first_seen_at, status, created_at, updated_at, agency_name, service_type, re_alert_flag) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
  ).run(
    "call-001",
...
test("call list includes normalized service type and re-alert flag", async () => {
  const db = setupDb();
  ...
  assert.equal(item.service_type, "EMS");
  assert.equal(item.re_alert, 1);
